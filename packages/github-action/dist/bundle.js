...
function extractIdentityAttribute(identityTag, name) {
  const regex = new RegExp(`\\b${name}\\s*=\\s*["']([^"']+)["']`, "i");
  const match2 = identityTag.match(regex);
  if (!match2 || !match2[1]) {
    return void 0;
  }
  return decodeXmlEntities(match2[1]);
...
function parseVsixManifestXml(xml) {
  ...
  return {
    ...
    publisher: identityTag ? extractIdentityAttribute(identityTag, "Publisher") : void 0,
    version: identityTag ? extractIdentityAttribute(identityTag, "Version") : void 0,
...
function ensureMetadataContainer(parsed) {
...
function setIdentityAttribute(metadata, attrName, value) {
  const identity = ensureObject(metadata.Identity);
  metadata.Identity = identity;
  identity[`@_${attrName}`] = value;
}
function setMetadataTextTag(metadata, tagName, value) {
...
function applyVsixManifestXmlMetadata(xml, manifestMods) {
...
function splitJsonAndXmlManifestMods(manifestMods) {
  const xmlMods = {
    id: manifestMods.id,
...
var import_fast_xml_parser, xmlParser, xmlBuilder, VsixWriter;
var init_vsix_writer = __esm({
  "packages/core/dist/vsix-writer.js"() {
    "use strict";
    import_fast_xml_parser = __toESM(require_fxp(), 1);
    xmlParser = new import_fast_xml_parser.XMLParser({
    ...
    xmlBuilder = new import_fast_xml_parser.XMLBuilder({
      ignoreAttributes: false,
      attributeNamePrefix: "@_",
      format: true,
      indentBy: "  ",
    ...
    VsixWriter = class _VsixWriter {
      ...
      async applyManifestModifications(reader, manifestPath, manifestMods, taskManifestMods, addedFiles) {
        ...
        const { jsonMods, xmlMods } = shouldSplitMetadata ? splitJsonAndXmlManifestMods(manifestMods) : { jsonMods: manifestMods, xmlMods: {} };
        ...
        if (hasVsixXmlManifest) {
          const hasXmlMods = !!xmlMods.id || !!xmlMods.publisher || !!xmlMods.version || !!xmlMods.name || !!xmlMods.description;
          if (hasXmlMods) {
            ...
            this.zipFile.addBuffer(Buffer3.from(updatedXml, "utf-8"), "extension.vsixmanifest");
