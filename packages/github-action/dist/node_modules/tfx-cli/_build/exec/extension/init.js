...
exports.ExtensionInit = exports.extractArchive = exports.resolveArchiveEntryPath = exports.getCommand = void 0;
...
function resolveArchiveEntryPath(initPath, archiveEntryName) {
    ...
    const relativeEntryName = firstSeparator >= 0 ? normalizedEntryName.substring(firstSeparator + 1) : normalizedEntryName;
...
exports.resolveArchiveEntryPath = resolveArchiveEntryPath;
async function ensureArchiveDestinationIsUnlinked(initPath, destinationPath) {
    const extractionRoot = path.resolve(initPath);
    const relativeDestination = path.relative(extractionRoot, destinationPath);
    ...
    for (const pathPart of relativeDestination.split(path.sep)) {
        ...
        let stats;
        try {
            stats = await (0, util_1.promisify)(fs.lstat)(currentPath);
        }
        catch (error) {
...
async function extractArchive(data, initPath) {
    ...
    for (const fileName of Object.keys(zip.files)) {
        ...
        await (0, mkdirp_1.default)(path.dirname(fullPath));
        await ensureArchiveDestinationIsUnlinked(initPath, fullPath);
        await (0, util_1.promisify)(fs.writeFile)(fullPath, buffer);
    }
}
...
class ExtensionInit extends extBase.ExtensionBase {
    ...
    async exec() {
        ...
        if (!noDownload) {
            ...
            try {
                const data = await (0, util_1.promisify)(fs.readFile)(downloadedZipPath);
