...
exports.validateTask = exports.validateRunner = exports.exists = exports.validate = void 0;
var fs = require("fs");
const path = __importStar(require("path"));
var check = require("validator");
var trace = require("./trace");
const deprecatedRunners = ["Node", "Node6", "Node10", "Node16"];
function readTaskJson(jsonFilePath) {
...
*/
function validate(jsonFilePath, jsonMissingErrorMessage, allMatchedPaths) {
    ...
    var issues = validateTask(jsonFilePath, taskJson);
    if (issues.length > 0) {
 ...
 */
function countValidRunners(taskData) {
 ...
 * @param allMatchedPaths optional array of all matched task.json paths for backwards compat detection
 */
function validateRunner(taskData, allMatchedPaths) {
    var _a, _b, _c;
    if (countValidRunners(taskData) == 0) {
        if (allMatchedPaths) {
            for (const matchedPath of allMatchedPaths) {
                ...
                if (taskData.name == matchedTaskData.name && taskData.id == matchedTaskData.id && ((_a = matchedTaskData.version) === null || _a === void 0 ? void 0 : _a.Major) > ((_b = taskData.version) === null || _b === void 0 ? void 0 : _b.Major)) {
        ...
        trace.warn("Task %s@%s is dependent on a task runner that is end-of-life and will be removed in the future. Please visit https://aka.ms/node-runner-guidance to learn how to upgrade the task.", taskData.name, ((_c = taskData.version) === null || _c === void 0 ? void 0 : _c.Major) || "?");
    }
}
...
/**
   ...
   */
function validateAllExecutionHandlers(taskPath, taskData, vn) {
    const issues = [];
    const executionProperties = ['execution', 'prejobexecution', 'postjobexecution'];
        ...
        if (taskData[executionType]) {
            Object.keys(taskData[executionType]).forEach(runner => {
                ...
                if (supportedRunners.indexOf(runner) === -1)
                    return;
                const target = (_a = taskData[executionType][runner]) === null || _a === void 0 ? void 0 : _a.target;
 ...
 */
function validateExecutionTarget(taskPath, vn, executionType, runner, target) {
    const issues = [];
    if (!target) {
